class UTPlusSavedInput extends Actor;

var UTPlusSavedInput Next;
var UTPlusSavedInput Prev;
...
function CopyFrom(float Delta, UTPlusPlayer P) {
	...
	bWalk = P.bRun != 0;
	bDuck = P.bDuck != 0;
	bJump = (P.aUp > 1.0) || P.UTPlus_PressedJumpSave;
	bDodg = false;//P.bPressedDodge;
...
function SerializeTo(UTPlusDataBuffer B, out float DeltaError) {
	local int Temp;
...
function DeserializeFrom(UTPlusDataBuffer B) {
	local int Temp;
...
function bool IsSimilarTo(UTPlusSavedInput Other) {
	return
		bLive == Other.bLive &&
		bForw == Other.bForw &&
		bBack == Other.bBack &&
